...
### draft → review/stable 검토 진행 방식

여러 문서의 status를 한 번에 검토받아야 할 때, **문서를 요약해서 보여주지
않는다.** 요약만 보고 승인하면 실제로 뭐가 쓰여있는지 확인 없이 도장 찍는
것과 같아서 검토 자체가 무의미해진다. 반드시 문서 **전체 본문**을 그대로
보여준 다음 stable/review/draft 유지 여부를 물어본다. 한 번에 처리할 문서
개수가 많다면 배치 크기를 줄이더라도(예: 한 번에 1~2개씩) 전체 내용을 보여주는
쪽을 우선한다.
