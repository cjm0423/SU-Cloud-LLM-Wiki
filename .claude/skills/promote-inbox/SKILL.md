---
name: promote-inbox
description: 00_Inbox의 미승격(status: raw) 원본을 정식 위키 문서(01_Concepts/02_QnA_Archive/03_Guides/04_Meetings/06_Decisions/07_SU-Cloud/08_Reference)로 추출·분류·연결·커밋한다. "inbox 처리해줘", "위키로 추출해줘", "원본 승격해줘" 같은 요청에 사용.
user-invocable: true
allowed-tools:
  - Read
  - Write
...
| SU-Cloud 프로젝트·환경 **자체**의 정보 (개요·로드맵·인프라 인벤토리·학내망 구조 등 "SU-Cloud가 무엇인가") | `07_SU-Cloud` | `99_Templates/OKF-SU-Cloud-Template.md` |
| 명령어·설정값·포트 등을 조회용으로만 모은 것 ("읽는 게 아니라 값·문법만 찾는" 자료) | `08_Reference` | `99_Templates/OKF-Reference-Template.md` |

> `08_Reference`는 대개 **여러 개념 문서에 흩어진 조회 정보를 모아** 만드는
> 것이라, 원본 1건을 1:1로 승격하는 경우는 드물다. 원본이 설명 위주면
> `01_Concepts`로, 순수 명령/설정 모음이면 `08_Reference`로 보낸다.
...
- 목적지 템플릿의 frontmatter 구조를 그대로 따른다. 폴더별 필수 필드:
  ...
  - `reviewed_by`/`reviewed_date`(Concept·Meeting·Guide 템플릿에 있음)는
    ...
    몫이다.
- **`tags`는 `TAXONOMY.md`의 23개 통제 어휘에서만 고른다.** 작성 전에
  `TAXONOMY.md`를 읽는다. 새 태그를 만들지 않는다 — 어휘에 없는 개념이면
  가장 가까운 상위 태그를 쓰고, 세부 내용은 본문에 적는다. 문서당 2~5개
  (대개 3~4개), `type`과 중복되는 태그(`#guide`/`#meeting`/`#concept`/
  `#decision`/`#qna`) 금지. 환경 축(`#campus-network`/`#devpc`/
  `#production`/`#prestudy`)은 판단 가능하면 하나 붙인다.
- 템플릿에 새로 생긴 정보 수집 섹션을 **비워두지 말고 채운다.** 원본에
  근거가 있는 것만 채우고, 근거가 없으면 섹션을 지우거나 "확인 필요"로
  표시한다 — 빈 placeholder를 그대로 남기지 않는다.
  - **Guide**: `verified_on`/`os`/`target_version`/`hardware`, 검증된 환경
    표, 최종 검증 체크리스트, 롤백 절차
  - **QnA**: `environment`/`symptoms`, 에러 메시지 **원문 그대로**(요약 금지
    — 검색 통로다), 시도했지만 실패한 방법, 재발 방지
  - **Meeting**: 결정 사항 표, 지적받은 피드백, Action Item 담당자 wikilink
  - **Decision**: `decision_status`, 판단 기준, 재검토 조건
...
- 확신 없는 정보를 임의로 지어내서 `related_nodes`나 본문에 채우지 않는다
  — 애매하면 사람에게 물어본다.
- `TAXONOMY.md`에 없는 태그를 새로 만들지 않는다. 어휘 자체를 늘려야 한다고
  판단되면 문서에 임의로 쓰지 말고 사람에게 제안한다.
