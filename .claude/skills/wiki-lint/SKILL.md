...
- **표 안의 wikilink는 파이프가 `\|`로 이스케이프되어 있다**
  (`00_Inbox/INDEX.md` 전체가 이 형태). `|`만 기준으로 자르면 링크명 끝에
  백슬래시가 붙어 122건이 통째로 오탐으로 잡힌다. `\|`와 `|`를 모두 구분자로
  처리할 것.
- **Notion 내보내기 원본에는 UTF-8 BOM(`EF BB BF`)이 붙어 있다.**
  `text.startswith("---")`로 frontmatter를 판별하면 `00_Inbox` 원본 대부분이
  "status 없음"으로 잘못 잡힌다. 파일을 읽을 때 `utf-8-sig`로 열 것.

### 2. INDEX.md 수동 목록 ↔ 실제 파일 불일치

`01_Concepts`, `02_QnA_Archive`, `03_Guides`, `04_Meetings`, `06_Decisions`, `07_SU-Cloud`, `08_Reference`
각각에 대해:
...
- `01_Concepts`, `03_Guides`, `04_Meetings`, `06_Decisions`, `07_SU-Cloud`, `08_Reference`: `draft` | `review` | `stable`
...
`TAXONOMY.md`를 읽어 허용 태그 23개 목록을 확보한 뒤, `01_Concepts`~
`06_Decisions`(및 `07_SU-Cloud`, `08_Reference`) 문서의 `tags`를 검사한다:

- 통제 어휘에 없는 태그 → 🔴 **미등록 태그** (매핑표에 대응하는 새 태그가
  있으면 함께 제시한다).
- `type`과 중복되는 태그(`#guide`, `#meeting`, `#concept`, `#decision`,
  `#qna`, `#people`, `#index`)나 폐기된 `#su-cloud` → 🔴 **폐기 태그**.
- 태그가 6개 이상 → 🟡 **과다 태깅** (태그로 요약하고 있다는 신호).
- `05_People`, `INDEX.md`에 주제 태그가 붙어 있으면 → 🟡 **불필요한 태그**.

역방향도 본다: 통제 어휘 중 **문서 2개 미만에만 붙은 태그**가 있으면
ℹ️ **죽은 태그 후보**로 보고한다 — 어휘를 다시 줄여야 한다는 신호다.

### 7. reviewed 필드 (참고용, 낮은 우선순위)

`status: "stable"`인 Concept/Meeting/Guide/Reference 문서 중 `reviewed_by` 또는
