---
name: review-status
description: draft/review 상태인 위키 문서(01_Concepts, 03_Guides, 04_Meetings, 06_Decisions, 07_SU-Cloud, 08_Reference)를 사람이 검토하도록 원문 전체를 보여주고 draft/review/stable 여부를 확인받는다. "status 검토해줘", "review 문서들 확인해줘", "draft 승인해줘" 같은 요청에 사용.
user-invocable: true
allowed-tools:
  - Read
  - Edit
...
`01_Concepts`, `03_Guides`, `04_Meetings`, `06_Decisions`, `07_SU-Cloud`, `08_Reference`의 `draft` /
...
`01_Concepts`, `03_Guides`, `04_Meetings`, `06_Decisions`, `07_SU-Cloud`, `08_Reference` 아래 `*.md`를
Glob/Grep으로 훑어 frontmatter `status: "draft"` 또는 `status: "review"`인
문서를 전부 나열한다. 파일명 + 제목 + 현재 status로 목록을 사람에게 먼저
보여준다.

이 배치에서 검토자가 누구인지 한 번만 물어본다 ("이번 검토는 누가
하시나요?"). `05_People/`에 해당 인물의 허브 노드가 있으면
`[[05_People/이름]]` wikilink로, 없으면 이름 그대로 기록해두고 이 배치의
모든 stable 승격에 재사용한다.
...
변경한다:
- 승인 → `status: "stable"`로 바꾸고, `reviewed_by`(1단계에서 확인한 검토자)와
  `reviewed_date`(오늘 날짜)를 채운다. 이 두 필드가 frontmatter에 없는
  구버전 문서라면 새로 추가한다.
- 수정 필요 표시 → `status: "review"` (reviewed_by/date는 건드리지 않는다 —
  아직 승인된 게 아니므로)
